...
async function gen_pure_unroll(N) {
  let a = new Array(N).fill(0);
  let b = new Array(N).fill(0);
  let c = new Array(N).fill(0);

  const fn = gen_generic_unroll(N);
...
async function gen_typed_unroll(N) {
  let a = new Float32Array(N).fill(0);
  let b = new Float32Array(N).fill(0);
  let c = new Float32Array(N).fill(0);
...
async function benchmark(Module, N) {
  const [pure_fn, p_a, p_b, p_c] = await gen_pure(N);
  const [pure_unroll_fn, pu_a, pu_b, pu_c] = await gen_pure_unroll(N);
  const [typed_fn, t_a, t_b, t_c] = await gen_typed(N);
  const [typed_unroll_fn, tu_a, tu_b, tu_c] = await gen_typed_unroll(N);
  const [emscripten_fn, e_a, e_b, e_c, emscripten_cleanup] = await gen_emscripten(Module, N);
  const [wasmblr_fn, w_a, w_b, w_c, wasmblr_cleanup] = await gen_wasmblr(Module, N, wasmblr_unroll);
  const [wasmblr_tuned_fn, wt_a, wt_b, wt_c, wasmblr_tuned_cleanup, unroll] = await gen_wasmblr_tuned(Module, N);
  ...
  const tu_gbs = perf(N, "  typed arrays (unroll):    ", typed_unroll_fn);
  ...
  return [p_gbs, pu_gbs, t_gbs, tu_gbs, e_gbs, w_gbs, wt_gbs];
}