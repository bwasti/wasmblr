...
struct MMGenNoSIMD : public wasmblr::CodeGenerator {
  MMGenNoSIMD(int M, int N, int K, int M_unroll, int N_unroll, int K_unroll) {
    assert(M_unroll <= M && "Invalid M unroll size");
    assert(N_unroll <= N && "Invalid N unroll size");
    ...
    auto fn = function({}, {}, [=]() {
      ...
      for (auto j = 0; j < K_unroll; ++j) {
        for (auto i = 0; i < M_unroll; ++i) {
          load_a.emplace_back(local(f32));
        }
        for (auto i = 0; i < N_unroll; ++i) {
          load_b.emplace_back(local(f32));
        }
      }
      ...
      for (auto m_unroll = 0; m_unroll < M_unroll; ++m_unroll) {
        for (auto n_unroll = 0; n_unroll < N_unroll; ++n_unroll) {
          local.get(c_off);
          f32.load(0, C_off + n_unroll * 4 + m_unroll * N * 4);
      ...
      for (auto k_unroll = 0; k_unroll < K_unroll; ++k_unroll) {
        for (auto m_unroll = 0; m_unroll < M_unroll; ++m_unroll) {
          local.get(a_off);
          f32.load(0, A_off + (m_unroll * K + k_unroll) * 4);
          local.set(load_a.at(m_unroll * K_unroll + k_unroll));
        }

        for (auto n_unroll = 0; n_unroll < N_unroll; ++n_unroll) {
          local.get(b_off);
          f32.load(0, B_off + (k_unroll * N + n_unroll) * 4);
      ...
      for (auto m_unroll = 0; m_unroll < M_unroll; ++m_unroll) {
        for (auto n_unroll = 0; n_unroll < N_unroll; ++n_unroll) {
          local.get(c_off);
          local.get(accs.at(m_unroll * N_unroll + n_unroll));
          f32.store(0, C_off + n_unroll * 4 + m_unroll * N * 4);
        }
      }

      local.get(c_off);
      i32.const_(N_unroll * 4);
      i32.add();
      local.set(c_off);
...
uint8_t *jit_mm_nosimd(int M, int N, int K, int Mu, int Nu, int Ku) {
  MMGenNoSIMD mm(M, N, K, Mu, Nu, Ku);
  auto bytes = mm.emit();
  uint8_t *out = (uint8_t *)malloc(bytes.size());
  memcpy(out, bytes.data(), bytes.size());
  return out;
...
int jit_mm_nosimd_len(int M, int N, int K, int Mu, int Nu, int Ku) {
  MMGenNoSIMD mm(M, N, K, Mu, Nu, Ku);
  auto bytes = mm.emit();
  return bytes.size();
}
