...
function buildGraph(times) {
  ...
  var color = d3.scaleOrdinal().domain(res).range(["#e41a1c", "#377eb8", "#4daf4a", "#ff7f00", "#984ea3", "#a65628", "#f781bf", "#999999"]);
...
function init(Module) {
  ...
  button.addEventListener("click", function() {
    ...
    remove(pre);
    let times = {};
    let keys = ["Pure JS", "Pure JS (unrolled)", "TypedArrays", "TypedArrays (unrolled)", "Emscripten (simd)", "Wasmblr", "Wasmblr (tuned)"];
    for (let k of keys) {
      times[k] = [];
    }
    (async () => {
